...
{
  pinMode(ECHO_PIN_1, INPUT); //Ultrasonic module initialization
  pinMode(TRIG_PIN_1, OUTPUT);

  pinMode(ECHO_PIN_2, INPUT); //Ultrasonic module initialization
  pinMode(TRIG_PIN_2, OUTPUT);

  pinMode(ECHO_PIN_3, INPUT); //Ultrasonic module initialization
  pinMode(TRIG_PIN_3, OUTPUT);

  pinMode(ECHO_PIN_4, INPUT); //Ultrasonic module initialization
  pinMode(TRIG_PIN_4, OUTPUT);

  pinMode(ECHO_PIN_5, INPUT); //Ultrasonic module initialization
  pinMode(TRIG_PIN_5, OUTPUT);

}
void DeviceDriverSet_ULTRASONIC::DeviceDriverSet_ULTRASONIC_Get(uint16_t *ULTRASONIC_Get_1, uint16_t *ULTRASONIC_Get_2 /*out*/)
{
  ...
  *ULTRASONIC_Get_1 = tempda_1;
  *ULTRASONIC_Get_2 = tempda_2;
  // sonar.ping() / US_ROUNDTRIP_CM; // Send ping, get ping time in microseconds (uS).
  
  Serial.print("ULTRASONIC_1=");
  Serial.print(*ULTRASONIC_Get_1); // Convert ping time to distance and print result (0 = outside set distance range, no ping echo)
  Serial.println("cm");

  // Serial.print("ULTRASONIC_2=");
  // Serial.print(*ULTRASONIC_Get_2); // Convert ping time to distance and print result (0 = outside set distance range, no ping echo)
  // Serial.println("cm");
}
